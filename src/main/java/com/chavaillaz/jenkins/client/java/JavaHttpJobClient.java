...
import com.chavaillaz.jenkins.domain.Folder;
import com.chavaillaz.jenkins.domain.JobInfo;
...
public class JavaHttpJobClient extends AbstractJavaHttpClient implements JobClient {
    ...
    public CompletableFuture<Void> createJob(Path path, String jobName, String configXML) {
        return sendAsync(requestBuilder(URL_JOB_CREATE, path, jobName).POST(ofString(configXML)), Void.class);
    }
    ...
    public CompletableFuture<Void> updateJobConfiguration(Path path, String jobName, String configXML) {
        return sendAsync(requestBuilder(URL_JOB_CONFIGURATION, path, jobName).POST(ofString(configXML)), Void.class);
    }

    @Override
    public CompletableFuture<Void> updateViewConfiguration(Path path, String viewName, String configXML) {
        return sendAsync(requestBuilder(URL_VIEW_CONFIGURATION, path, viewName).POST(ofString(configXML)), Void.class);
    }
    ...
    public CompletableFuture<Void> renameJob(Path path, String jobName, String newName) {
        return sendAsync(requestBuilder(URL_JOB_RENAME, path, jobName, newName)
                .setHeader(HEADER_CONTENT_TYPE, HEADER_CONTENT_FORM)
                .POST(ofFormData(Map.of("newName", newName))), Void.class);
    }
    ...
    public CompletableFuture<Void> buildJob(Path path, String jobName, Map<Object, Object> properties) {
        return sendAsync(requestBuilder(URL_JOB_BUILD_PARAMETERS, path, jobName, queryFromKeyValue(properties)).POST(ofFormData(properties)), Void.class);
    }
